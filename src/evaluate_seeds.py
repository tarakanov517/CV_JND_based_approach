...
MODELS = {
    ...
    "lateral_high": "model_lateral_high.tar",
    "dendrite_block1_low": "model_dendrite_block1_low.tar",
    "all_parameter_high": "model_all_parameter_high.tar",
}
...
def load_attack_data(model, path, attack_name):
    attack = make_fgsm(model) if attack_name == "fgsm" else make_pgd(model, 10)
...
def main():
    ...
    parser.add_argument("--root", default="human_noise_seeds")
    ...
    with open(raw_path, "w", newline="", encoding="utf-8") as raw_file:
        ...
        for model_seed in seeds:
            for model_name, filename in MODELS.items():
                ...
                whitebox_attacks = (
                    ("fgsm", make_fgsm(model)),
                    ("pgd10", make_pgd(model, 10)),
                    ("pgd50", make_pgd(model, 50)),
                )
